...
# Sidebar contents
with st.sidebar:
    st.set_page_config(page_title="MussieGPT")
    st.title('MussieGPT')
    st.markdown('''
    This page is dedicated to brontos *Kalab* and *Gerry* who kept me motivated while mocking my MussieGPT :)
...
def main():
    st.header('Welcome to MussieGPT')
    # Upload a PDF file

    st.write('''
    Ok, brontos, MussieGPT is finally here! Upload a PDF document and ask any question about the document.
    For instance, you can upload a document that you are well aware of, for instance your CV, and
    ask questions that you may think can be answered from the document.

    This is neither a simple search button on PDF reader nor an app that functions perfectly. In the data
    science field, you just need a different techniques to make this work and we tried to experiment the
    technique and build the app for fun. Just have fun trying/mocking/breaking it and please let me know
    your opinion on it.''')
